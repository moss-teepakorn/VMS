...
export async function exportPaymentReportPdf({ title, fileName, columns, rows, filter, sumAmount }) {
  ...
  iframe.style.cssText = 'position:fixed;left:-9999px;top:0;border:none;width:900px;height:1200px;';
  ...
  const canvas = await html2canvas(el, { scale: 2, useCORS: true, backgroundColor: '#fff', width: 900 });
  // 5. ใส่ลง jsPDF
